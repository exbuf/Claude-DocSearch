...
The assistant adds value when it **reads and reasons over** what peekdocs found — not when it merely relays a hit list. For a bare keyword search ("show every line with *invoice*") the assistant adds little over peekdocs itself; peekdocs already does the finding, exactly and deterministically. So phrase requests as *questions*, and know which of two kinds you're asking — a **needle** question (the answer lives in a few specific hits, like finding a needle in a haystack) or a **census** question (the answer needs the *whole* set counted or listed, like taking a census):
...
The server speaks over **stdio**, the transport every MCP client uses, and `--root` confines it to the folders you're willing to expose. Your MCP client normally launches it for you (see the [Quickstart](#quickstart-claude-code-the-fastest-way-to-try-it-but-cloud-based-and-not-private) below) — the command it runs is:
...
### Quickstart: Claude Code (the fastest way to try it but cloud-based and not private)
...
- **Claude Code:** `claude mcp remove peekdocs`, then re-add with the new `--root` (see [Quickstart](#quickstart-claude-code-the-fastest-way-to-try-it-but-cloud-based-and-not-private)).
...
> **Prefer a guided walkthrough?** The [Local AI Assistant beginner's guide](LOCAL_AI_SETUP.md) covers this local setup end to end, with troubleshooting.
...
> **Keep `QUERY` narrow — every match goes to the model.** peekdocs feeds *all* the matched lines into the prompt, and a local model's context window is limited: a broad term (say `BDNF`, or `renew`) can return hundreds of matches that overflow the window, so the model silently sees only a fraction — and because the "cite the file" instruction sits at the *top* of the prompt, it gets truncated away and the answers come back **uncited**. The script already carries two safety nets: **`-m 30`** caps how many matches peekdocs emits (raise it, or use `-m 0` for no limit), and **`options:{num_ctx:8192}`** widens the model's window so the whole prompt fits. (These two are a matched pair: `-m 0` on a broad term only works if you *also* raise `num_ctx` to hold every match — and even then hundreds of matches yield a thematic *overview*, not per-line citations, so for a complete verifiable list read peekdocs's own report instead.) But the real control is a **narrow query** — `-m` keeps only the *first* N matches in scan order, so a broad term makes them cluster in one file; a specific phrase (`auto-renewal`), or pointing `FOLDER` at a subfolder, gives you fewer, on-topic matches spread across your documents. If you genuinely need *all* of them, that is a **census** question — ask peekdocs directly instead.
>
...
peekdocs keeps a single, comprehensive glossary in **[docs/GLOSSARY.md](GLOSSARY.md)** — every peekdocs term (flags, search modes, error names, file-handling and packaging internals, plus industry context and longer-form backgrounds) in one place, so there is one source of truth to keep current. Open it and use your browser's find (Cmd/Ctrl+F) to jump to a term.
